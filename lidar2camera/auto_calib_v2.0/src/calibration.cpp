...
Calibrator::Calibrator(const std::string mask_dir, const std::string lidar_file, const std::string calib_file,
                       const std::string img_file, const std::string error_file)
{
  // load calib file
...
void Calibrator::ProcessPointcloud(const pcl::PointCloud<pcl::PointXYZI>::Ptr pc_origin)
{
  ...
  {
    if (!std::isfinite(src_pt.x) || !std::isfinite(src_pt.y) || !std::isfinite(src_pt.z)) continue;
    Eigen::Vector4f vec;
  ...
  if (intensity_max > 1) intensity_max /= 2;
...
void Calibrator::Segment_pc(const pcl::PointCloud<pcl::PointXYZI>::Ptr cloud, pcl::PointCloud<pcl::Normal>::Ptr normals,
                            std::vector<pcl::PointIndices> &seg_indices)
{
  // compute_normals
  ...
  while (plane_size > 2000)
  {
    ...
    indices_plane_all->indices.insert(indices_plane_all->indices.end(), indices_plane->indices.begin(),
...
void Calibrator::Calibrate(const std::string &folder)
{
  VisualProjection(init_extrinsic_, img_file_, folder + "/init_proj.png");
  VisualProjectionSegment(init_extrinsic_, img_file_, folder + "/init_proj_seg.png");
  VisualProjection(extrinsic_, img_file_, folder + "/error_proj.png");
  VisualProjectionSegment(extrinsic_, img_file_, folder + "/error_proj_seg.png");
  ...
  PrintCurrentError();
  VisualProjection(extrinsic_, img_file_, folder + "/refined_proj.png");
  VisualProjectionSegment(extrinsic_, img_file_, folder + "/refined_proj_seg.png");
}
...
bool Calibrator::CalScore(Eigen::Matrix4f T, float &score, bool is_coarse)
{
  ...
  for (int i = 0; i < N_MASK; i++)
  {
    ...
    if (mask_point_num_[i] < min_mask_point_num) continue;
    int points_on_mask = mask_intensity[i].size();
    if (points_on_mask < 20) continue;
  ...
  // std::cout << "score: " << normal_score << " " << intensity_score << " " << segment_score << " " << score <<
  // std::endl;
  return true;
}
...
void Calibrator::BruteForceSearch(int rpy_range, float rpy_resolution, int xyz_range, float xyz_resolution,
                                  bool is_coarse)
{
  std::cout << "Start brute-force search around [-" << rpy_range * rpy_resolution << "," << rpy_range * rpy_resolution
            << "] degree and [-" << xyz_range * xyz_resolution << "," << xyz_range * xyz_resolution << "] m"
            << std::endl;
  float best_var[6] = {0};
  float score;
  std::vector<Var6> vars;
  ...
  for (auto var : vars)
  {
    ...
    if (score > max_score_)
    {
      ...
      std::cout << "match score increase to: " << max_score_ << ", "
                << "var:" << best_var[0] << " " << best_var[1] << " " << best_var[2] << " " << best_var[3] << " "
...
{
  std::cout << "Start random search around [-" << rpy_range << "," << rpy_range << "] degree and [-" << xyz_range << ","
            << xyz_range << "] m" << std::endl;
  float var[6] = {0};
  float bestVal[6] = {0};
  ...
  for (int i = 0; i < search_count; i++)
  {
    ...
    if (score > max_score_)
    {
      ...
      std::cout << "match score increase to: " << max_score_ << ", "
                << "val:" << bestVal[0] << " " << bestVal[1] << " " << bestVal[2] << " " << bestVal[3] << " "
...
void Calibrator::PrintCurrentError()
{
  ...
  std::cout << "Error:" << RAD2DEG(Util::GetRoll(error_T)) << " " << RAD2DEG(Util::GetPitch(error_T)) << " "
            << RAD2DEG(Util::GetYaw(error_T)) << " " << Util::GetX(error_T) << " " << Util::GetY(error_T) << " "
            << Util::GetZ(error_T) << std::endl;
}

Eigen::Matrix4f Calibrator::GetFinalTransformation() { return extrinsic_; }
...
bool Calibrator::ProjectOnImage(const Eigen::Vector4f &vec, const Eigen::Matrix4f &T, int &x, int &y, int margin)
{
  ...
  if (vec2(2) <= 0) return false;
  x = (int)cvRound(vec2(0) / vec2(2));
  y = (int)cvRound(vec2(1) / vec2(2));
  if (x >= -margin && x < IMG_W + margin && y >= -margin && y < IMG_H + margin) return true;
  return false;
}
