...
POINT_CLOUD_REGISTER_POINT_STRUCT(PointXYZINS,
                                  (float, intensity, intensity)(float, curvature, curvature)(int, segment, segment))

class Calibrator
{
 public:
  Calibrator(const std::string mask_dir, const std::string lidar_file, const std::string calib_file,
             const std::string img_file, const std::string error_file);
  void Calibrate(const std::string &folder);
  ...
  void Segment_pc(const pcl::PointCloud<pcl::PointXYZI>::Ptr pc_origin, pcl::PointCloud<pcl::Normal>::Ptr normals,
