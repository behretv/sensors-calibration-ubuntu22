...
int main(int argc, char *argv[])
{
  ...
  const std::string folder = std::filesystem::path(calib_file).parent_path().string();
  const std::string filename = folder + "/refined_extrinsics.txt";
  ...
  for (auto iter = refined_extrinsics.begin(); iter != refined_extrinsics.end(); iter++)
  {
    ...
    file_stream << "(Roll,Pitch,Yaw,tx,ty,tz): " << euler_angles.transpose() << " " << translation.transpose()
                << std::endl;
  }
  file_stream.close();
    std::cout << "\033[94mSaved: " << filename <<"\033[0m" << std::endl;
