...
void LoadCalibFile(const std::string &filename,
    ...
    while (getline(file, line)) {
        ...
        getline(file, line);
        ss = std::stringstream(line);
        ss >> tmpStr >> extrinsic.euler_angles[0] >>
            extrinsic.euler_angles[1] >> extrinsic.euler_angles[2] >>
            extrinsic.t_matrix[0] >> extrinsic.t_matrix[1] >>
            extrinsic.t_matrix[2];
...
int main(int argc, char *argv[]) {
    if (argc != 3) {
        std::cout
            << "Usage: ./run_lidar2lidar <lidar_file>.txt <calib_file>.txt"
            << std::endl;
        return 0;
    }
    ...
    file_stream.open("config/refined_extrinsics.txt");
    std::map<int32_t, Eigen::Matrix4d> refined_extrinsics =
        calibrator.GetFinalTransformation();
    for (auto iter = refined_extrinsics.begin();
         iter != refined_extrinsics.end(); iter++) {
        int32_t slave_id = iter->first;
        Eigen::Matrix4d transform = iter->second;
        auto translation(transform.block<3, 1>(0, 3));
        auto euler_angles =
            Eigen::Matrix3d(transform.block<3, 3>(0, 0)).eulerAngles(0, 1, 2);

        // Create file
        file_stream << "device_id: " << slave_id << std::endl;
        file_stream << "(Roll,Pitch,Yaw,tx,ty,tz): " << euler_angles.transpose()
